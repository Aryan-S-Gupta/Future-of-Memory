...
def show_database_state():
    ...
    for session in sessions:
        ...
        for turn in turns:
            ...
            print(f"      Question: {turn.question[:50] if turn.question else 'None'}...")
            
            for option in options:
                completeness = []
                if option.image_text: completeness.append("I")
                if option.scenario: completeness.append("S")
                comp_str = "(" + "/".join(completeness) + ")" if completeness else "(empty)"
                print(f"         {option.label}: {option.option_text[:30]}... {comp_str}")
...
def main():
    ...
    if session_id:
        ...
        try:
            ...
            if options.count() >= 2:
                ...
                turn.save()
                print(f"Set user choice: Option {chosen_option.label} for continuation test\n")
    ...
    if all_passed:
        ...
        print("  All three generation steps (Question → Image Text → Scenario) integrated successfully!")
    else:
